...
class DuplicateMenu
{
    ...
    public function execute(
        ...
        $payload = [
            ...
            'description' => $options['description'] ?? $menu->description,
            'type' => $options['type'] ?? $menu->type,
            'status' => 'draft',
            'default_guest_count' => $options['default_guest_count'] ?? $menu->default_guest_count,
            'event_id' => $options['target_event_id'] ?? null,
            'sections' => [],
        ];

        if (is_array($options['sections'] ?? null)) {
            $payload['sections'] = $options['sections'];
        } elseif ($includeSections) {

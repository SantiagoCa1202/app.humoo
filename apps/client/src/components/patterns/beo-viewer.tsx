...
export function BEOViewer({ accessibilityLabel, compact = false, document, error, loading = false, mode = "structured", onFieldPress, onModeChange, originalContent, selectedFieldId, structuredData, structuredSections, version }: BEOViewerProps) {
  ...
  const canShowOriginal = Boolean(originalContent || document);
  ...
  return (
    <View accessibilityLabel={accessibilityLabel ?? t("documents.beoViewer.accessibilityLabel")} style={{ gap: theme.spacing[3] }}>
      ...
      {resolvedMode === "original" ? (originalContent ?? (document ? <DocumentCard compact document={document} latestVersion={version} /> : null)) : (

...
export function BEOVersionCard({ accessibilityLabel, actions, compact = false, isCurrent = false, onCompare, onPress, selected = false, version }: BEOVersionCardProps) {
  ...
  const trailing = <View>{isCurrent ? <Badge label={t("documents.beoVersion.current")} size="sm" variant="primary" /> : actions}</View>;
  return <EntityCard accessibilityLabel={accessibilityLabel ?? t("documents.beoVersion.accessibilityLabel", { version: title })} eyebrow={<DocumentSourceChip source={version.source} />} metadata={metadata} onLongPress={onCompare ? () => onCompare(version) : undefined} onPress={onPress} selected={selected} status={version.status as never} statusNamespace={version.status ? "beoVersions" : undefined} title={title} trailing={trailing} />;
}
